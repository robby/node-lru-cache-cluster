...
var LRUCacheProxy = function LRUCacheProxy(options) {
	if (!(this instanceof LRUCacheProxy)) {
		if(cluster.isMaster) {
			...
			var lruPeek = LRUCache.prototype.peek;

			lru.get = function(key, callback) {
				callback(lruGet.apply(lru, arguments));
			};
			
			lru.peek = function(key, callback) {
				callback(lruPeek.apply(lru, arguments));
			};
...
LRUCacheProxy.prototype.peek = function(key, callback) {
	var failSafe = setTimeout(function() { failSafe = undefined; callback(undefined); }, 100);

	send(this.namespace, 'peek', key, function(result) {
		if(!failSafe) return;
